...
while read -r user_at_host; do
    ...
    while read -r file; do
        ...
        (cat "$EXCLUDE_PATTERNS"; getDeletedOrNewFiles "$local_file") | sed "s\\^$local_file\\\\"  | rsync -aut "$RSYNC_EXTRA_FLAGS" --mkpath --delete --exclude-from - "$user_at_host$file" "$local_file" || true
